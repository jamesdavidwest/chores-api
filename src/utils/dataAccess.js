...
function calculateFutureDates(startDate, endDate, frequencyId) {
  try {
    ...
    const maxIterations = 365; // Maximum one year of dates
    let iterations = 0;
    ...
    console.log(
      `Generated ${dates.length} dates for chore with frequency ${frequencyId}`
    );
    return dates;
  } catch (error) {
  ...
  try {
    ...
    if (filters.choreId) {
      instances = instances.filter(
        (instance) => instance.chore_id === parseInt(filters.choreId)
      );
    }

    if (filters.startDate) {
      instances = instances.filter(
        (instance) => instance.due_date >= filters.startDate
      );
    }

    if (filters.endDate) {
      instances = instances.filter(
        (instance) => instance.due_date <= filters.endDate
      );
    }
...
async function updateChoreInstance(id, updates) {
  ...
  const index = db.chore_instances.findIndex(
    (instance) => instance.id === parseInt(id)
  );
  ...
  try {
    ...
    if (filters.userId !== undefined) {
      chores = chores.filter(
        (chore) => chore.assigned_to === parseInt(filters.userId)
      );
    }

    if (filters.locationId) {
      chores = chores.filter(
        (chore) => chore.location_id === parseInt(filters.locationId)
      );
    }
    ...
    if (filters.includeInstances) {
      ...
      chores = chores.map((chore) => ({
        ...chore,
        instances: instances.filter(
          (instance) => instance.chore_id === chore.id
        ),
      }));
    }
...
async function createChore(choreData) {
  ...
  try {
    const db = await readDatabase();
    console.log(
      "createChore - Current database state:",
    ...
    console.log(
      "createChore - New chore object:",
      JSON.stringify(newChore, null, 2)
    );
