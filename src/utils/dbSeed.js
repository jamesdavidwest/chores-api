...
async function seedUsers(db) {
    const users = [
        ...
        { name: 'Parent Manager', email: 'parent@example.com', role: 'MANAGER', password: 'password' },
        { name: 'Child User', email: 'child@example.com', role: 'USER', password: 'password' }
    ];
...
async function seedFrequencyTypes(db) {
    ...
    for (const freq of frequencies) {
        await run(
            db,
            'INSERT INTO frequency_types (name) VALUES (?)',
            [freq.name]
        );
    }
...
async function seedTasks(db) {
    ...
    for (const task of tasks) {
        ...
        const frequencyId = await getIdByName(db, 'frequency_types', task.frequency);
        ...
        await run(
            ...
            [
                task.name, locationId, frequencyId, categoryId,
                userId, task.points_value, '09:00:00'
            ]
        );
    }
}
