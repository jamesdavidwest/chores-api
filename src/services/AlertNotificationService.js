...
class AlertNotificationService extends EventEmitter {
    ...
    getActiveAlerts() {
        return Array.from(this.alerts.values())
            .filter(alert => alert.state === ALERT_STATE.ACTIVE);
    }
    ...
    _evaluateSystemMetrics(metrics) {
        ...
        if (metrics.cpu && metrics.cpu.usage) {
            ...
            if (cpuUsage >= config.cpu.critical) {
                this._createAlert('CPU_CRITICAL', 'CPU usage critical', {
                    value: cpuUsage,
                    threshold: config.cpu.critical
                }, SEVERITY.CRITICAL);
            } else if (cpuUsage >= config.cpu.error) {
                this._createAlert('CPU_ERROR', 'CPU usage high', {
                    value: cpuUsage,
                    threshold: config.cpu.error
                }, SEVERITY.ERROR);
            } else if (cpuUsage >= config.cpu.warning) {
                this._createAlert('CPU_WARNING', 'CPU usage elevated', {
                    value: cpuUsage,
        ...
        if (metrics.memory && metrics.memory.usagePercent) {
            ...
            if (memoryUsage >= config.memory.critical) {
                this._createAlert('MEMORY_CRITICAL', 'Memory usage critical', {
                    value: memoryUsage,
                    threshold: config.memory.critical
                }, SEVERITY.CRITICAL);
            } else if (memoryUsage >= config.memory.error) {
                this._createAlert('MEMORY_ERROR', 'Memory usage high', {
                    value: memoryUsage,
                    threshold: config.memory.error
                }, SEVERITY.ERROR);
            } else if (memoryUsage >= config.memory.warning) {
                this._createAlert('MEMORY_WARNING', 'Memory usage elevated', {
                    value: memoryUsage,
    ...
    _evaluateApplicationMetrics(metrics) {
        ...
        if (metrics.responseTime && metrics.responseTime.average) {
            ...
            if (avgResponseTime >= config.responseTime.critical) {
                this._createAlert('RESPONSE_TIME_CRITICAL', 'Response time critical', {
                    value: avgResponseTime,
                    threshold: config.responseTime.critical
                }, SEVERITY.CRITICAL);
            } else if (avgResponseTime >= config.responseTime.error) {
                this._createAlert('RESPONSE_TIME_ERROR', 'Response time high', {
                    value: avgResponseTime,
                    threshold: config.responseTime.error
                }, SEVERITY.ERROR);
            } else if (avgResponseTime >= config.responseTime.warning) {
                this._createAlert('RESPONSE_TIME_WARNING', 'Response time elevated', {
                    value: avgResponseTime,
    ...
    _evaluateDatabaseMetrics(metrics) {
        ...
        if (metrics.poolStatus) {
            const poolUsage = (metrics.poolStatus.used / metrics.poolStatus.total) * 100;
            if (poolUsage >= config.connectionPool.critical) {
                this._createAlert('DB_POOL_CRITICAL', 'Database connection pool usage critical', {
                    value: poolUsage,
                    threshold: config.connectionPool.critical
                }, SEVERITY.CRITICAL);
            } else if (poolUsage >= config.connectionPool.error) {
                this._createAlert('DB_POOL_ERROR', 'Database connection pool usage high', {
                    value: poolUsage,
                    threshold: config.connectionPool.error
                }, SEVERITY.ERROR);
            } else if (poolUsage >= config.connectionPool.warning) {
                this._createAlert('DB_POOL_WARNING', 'Database connection pool usage elevated', {
                    value: poolUsage,
        ...
        if (metrics.responseTime && metrics.responseTime.average) {
            ...
            if (avgQueryTime >= config.queryTime.critical) {
                this._createAlert('DB_QUERY_TIME_CRITICAL', 'Database query time critical', {
                    value: avgQueryTime,
                    threshold: config.queryTime.critical
                }, SEVERITY.CRITICAL);
            } else if (avgQueryTime >= config.queryTime.error) {
                this._createAlert('DB_QUERY_TIME_ERROR', 'Database query time high', {
                    value: avgQueryTime,
                    threshold: config.queryTime.error
                }, SEVERITY.ERROR);
            } else if (avgQueryTime >= config.queryTime.warning) {
                this._createAlert('DB_QUERY_TIME_WARNING', 'Database query time elevated', {
                    value: avgQueryTime,
