...
class InstanceService {
  ...
  async getById(id) {
    try {
      const instance = await this.db(this.tableName).where("id", id).first();

      if (!instance) {
        throw new AppError(ErrorTypes.NOT_FOUND, this.serviceName, "getById", {
          resource: "Instance",
          id: id,
        });
      }
  ...
  async update(id, instanceData) {
    try {
      return await transactionManager.executeTransaction(async (trx) => {
        ...
        if (!currentInstance) {
          throw new AppError(ErrorTypes.NOT_FOUND, this.serviceName, "update", {
            resource: "Instance",
            id: id,
  ...
  async delete(id) {
    try {
      return await transactionManager.executeTransaction(async (trx) => {
        const instance = await trx(this.tableName).where("id", id).first();

        if (!instance) {
          throw new AppError(ErrorTypes.NOT_FOUND, this.serviceName, "delete", {
            resource: "Instance",
            id: id,
        ...
        // Delete the instance
        await trx(this.tableName).where("id", id).del();
